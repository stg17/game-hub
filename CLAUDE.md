...
4. Add an `<a class="board board--<slug>">` to the `.tray` in root `index.html`, **immediately before `<div class="slot">`**, copying any existing board's structure exactly: `.board__field` wrapping an inline `.board__art` SVG plus `.board__no`, then `.board__foot` with `.board__title` / `.board__rule` / `.board__spec` / `.board__record[data-record="<key>"]`. Type a literal `No. NN` in the stamp as the no-JS fallback; `hub.js` overwrites it from position.
5. Draw the board art in the house hand: a `0 0 250 250` viewBox, `stroke="currentColor"` at weight 5, paint set on a parent `<g>` with bare geometry inside, `fill="none"` explicit on stroked paths, a real `role="img"` + `aria-label`. No emoji and no icon-font glyphs anywhere in this project. **The art carries no words** — no captions, labels or spec blocks inside the drawing; whatever needs saying is said by the `board__rule` and `board__spec` lines printed under it. Digits are allowed only where they are the subject of the drawing — 2048’s tile values, the numerals and arithmetic signs strewn across Math Puzzles — never as a caption or a label.
6. Add one line to `hub.css`: `.board--<slug> { --field: var(--f-<slug>); --on-field: …; grid-column: span n; --art-cap: Npx; }`, plus the `--f-<slug>` token in `:root`. **The tray is a hand-composed plan, not a cycle** — pick the span so its row still adds up to 12, adjust `.slot`'s span so the last row still closes, and repeat for the `≤1080px` block. Cards in a row stretch to one height, so the `--art-cap` values in a row also decide which board sets that height: the tallest natural card wins, and 2048's `aspect-ratio: 1` is tuned to be the tallest in its row so its square is exact.
...
8. Update this file's **Games** section below, and add the game's field colour and set number to `DESIGN.md`. Reordering the tray renumbers every board after the one you moved — `hub.js` does that live, but the literal `No. NN` fallbacks in the markup, `DESIGN.md`'s colour list and `.impeccable/design.json`'s display names all have to be moved by hand.
...
**Pausing mid-level**: `PAUSED` is itself a small menu (`Game.PauseOptions`: Continue / Restart / Menu), navigated with the same up/down-arrow + Enter or mouse-click pattern as the main menu, plus Escape/P as a one-key resume shortcut. The way in with a mouse is the chip `ui.js` prints on the HUD rail beside the score (`hud-pause`, drawn only while `showPause` — under the pause sheet it would be a control you cannot press); `updatePlaying` treats that click exactly as it treats Escape. It carries the two struck bars of `pauseMark`, not the word: `hudButton` takes a `paint(ctx, cx, cy, colour)` callback rather than a label, and the mark is drawn like everything else in the box — procedurally, never as a glyph from a font. Restart Level calls `Game.loadLevel(Game.levelIndex)` again with `Game.lives` reset to 3 and `Game.score` rolled back to `Game.levelStartScore` (snapshotted in `Game.loadLevel` the moment a level begins) — so a restart is a clean redo of just the current level, not a full new run.
...
**Only the live device lights anything.** A keyboard selection and a mouse hover both print red, so if both could light at once a menu would show two chosen options — which it did. `Game.Input.pointerActive()` says which device is being used (true on mouse move or click, false the moment any key goes down, false when the pointer leaves the board); `ui.js`'s `isHovered` returns false unless it is true, and `game.js`'s `selectionUnderPointer(prefix)` walks the selection index to whatever the pointer is over while it is. The two halves are what keep it to one lamp: the pointer takes the selection with it rather than lighting a second thing, and a key press puts the hover out rather than leaving a stale one lit. Enter therefore always acts on the option that is printed red. Any new screen with a moving selection needs both halves.
...
**State machine** (`app.js`, no game loop — pure event-driven, `setInterval` only for the visible Timed-mode readout): `menu → settings → playing → solved`, back-linked at every stage. Leaving `playing` without solving — the "All puzzles" arrow under the title, or Deal Another — calls `Storage.recordAbandon`, which resets that puzzle-type+difficulty's streak but never its lifetime solved count or best time. There is one way back and it is always in the same place: the arrow is shown on every screen except the menu, and the settings screen no longer carries a Back button of its own. The running head is shown only once a puzzle is actually out — menu and settings hide it, because there is nothing in progress for it to report and it was drawing an empty badge.
...
**Make 24** (`make24.js`): click-to-combine tile UI (click tile, click op, click another tile → replaced by one result tile), not free-text parsing — avoids order-of-operations ambiguity entirely. All arithmetic is exact reduced-fraction math via `rational.js` (`Rational.add/sub/mul/div/equals`), never floats, so e.g. `8/(3-8/3)=24` checks out exactly. The solver (`combineAll` + recursive `countSolutions`/`findHint`) works by picking any pair from the current value list and recursing on the reduced list — every parenthesization shape falls out of that one loop, no shape enumeration needed. Difficulty is bucketed by solution count from rejection-sampled random draws (Easy ≥4 solutions, Medium 2-3, Hard exactly 1), with a guaranteed-solvable `[1,2,3,4]` fallback if 300 attempts never hit a bucket. **Check is hidden for this type and for Number Pyramid** (`app.js`, in `startPuzzle`): both renders call `onSolved` themselves the moment the puzzle comes out right — the last tile reading 24, or the last block filled correctly — so the button could never report a win, only ever "not solved yet", which the player can already see. Calcudoku keeps its Check, where a part-filled grid makes the question genuinely worth asking.
...
**Number Pyramid** (`pyramid.js`): each cell equals the sum of the two cells below it (`buildPyramidFromBottom`). The rule is printed once, on the menu card that sends you here, rather than again over the pyramid itself. Generation starts fully revealed and greedily un-reveals cells in random order, keeping each removal only if a constraint-propagation solver (`propagate` — repeatedly fills in any parent/child triple with exactly one unknown, to a fixed point) can still fully deduce the pyramid without it. Hard difficulty allows negative bottom-row values, since negative-number arithmetic is still 7th-grade-legal.
...
**Nothing ever pauses at the bottom.** `lockPiece` scores, collapses the rows (`Board.clearRows`) and spawns the next piece **on the same frame**, so a landing that clears four lines costs exactly what a landing that clears none costs: nothing. There is no `CLEARING` state and no `finishClear` — an earlier build parked the game for `CLEAR_FLASH` seconds while the rows strobed, and that freeze was the one place the board stopped answering.

What survives is an afterimage: `flashRows`/`flashLeft` drive a pale band the renderer fades over the rows where the lines were, and `flashLeft` is read only by `draw`. **Nothing waits on it** — it is decremented inside the `PLAYING` branch of `update` (so a paused game doesn't burn the fade off behind the overlay) and it gates no transition. If you add to this, keep it that way: the moment a visual effect can hold up a spawn, the pause is back.

`LOCK_DELAY` is 0.15s, short enough that a landing reads as instant while a slide or rotation already in progress still lands. A deliberate late adjustment doesn't, and that is the trade — don't raise it back without saying so.

...
**State machine** (`app.js`, no game loop — pure event-driven, `setInterval` only for the visible Timed-mode readout): `menu → settings → playing → solved`, back-linked at every stage. Leaving `playing` without solving — the "All puzzles" arrow under the title, or Deal Another — calls `Storage.recordAbandon`, which resets that puzzle-type+difficulty's streak but never its lifetime solved count or best time.

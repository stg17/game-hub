...
**The opening.** The hub has one authored set piece, ~3.1s, built in `hub.js` and played once per browser session: a closed tray sits on the table, its cover lifts off, the cloth over the contents fades to show the boards already inside, the view flies into the box until the boards are full size, and the cover lands in the top-left corner as the title tag.

The box is not a prop beside the page — it *is* the page. `.boot-walls` is drawn at `inset: -46px` around the real `.boot-stage`, and `.boot-cloth` covers its contents from within, so both scale with it. The whole flight is one transform on the stage running from a fit-to-viewport scale back to `none`: that is why the boards finish at exactly their resting size, and why the chipboard leaves the frame instead of dissolving in place. The walls' interior is the same green as the table, so by the time they fade they are both off-screen and indistinguishable from what is behind them. Nothing inside the box moves on its own after the reveal — the contents are static and the motion is all camera.

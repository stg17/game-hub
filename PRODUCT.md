...
- Verification is mostly manual — reload and play. There is no lint or repo-wide test command, but two games ship committed checks that run with plain `node` and no dependencies: `selfcheck.js` and `playcheck.js` in both `games/nonogram/` and `games/terms-and-conditions/`. They exist because those two games make a promise worth proving — every plate solvable by deduction, every clause matching its printed English. The other five games have no automated coverage.

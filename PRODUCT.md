...
- Persistence is `localStorage` only: `dcromp_unlocked`, `ttt_scores_v2`, `mathpuzzles_stats_v1`, `tetris_best_v1`, `2048_state_v1`, `terms_stats_v1`, `inkbynumbers_stats_v1` (Nonograms — the key predates the rename and is frozen so recorded times survive it). 2048 keeps separate bonus saves in `2048_state_3_v1` / `2048_state_5_v1` and remembers the grid choice in `2048_size_v1`.
- Verification is mostly manual — reload and play. There is no lint or repo-wide test command, but two games ship committed checks that run with plain `node` and no dependencies: `selfcheck.js` and `playcheck.js` in both `games/nonograms/` and `games/terms-and-conditions/`. They exist because those two games make a promise worth proving — every plate solvable by deduction, every clause matching its printed English. The other five games have no automated coverage.
- Seven games ship today: D.C. Romp (platformer, 5 levels), Tic Tac Toe (2-player or vs a beatable computer, scoreboard per opponent), Math Puzzles (Make 24 / Calcudoku / Number Pyramid, 3 difficulties, timed toggle), Tetris (SRS, hold, ghost), 2048 (undo, classic 4×4 plus 3×3 and 5×5 bonus grids; 5×5 adds two tiles per successful move when space allows), Terms & Conditions (reaction, overriding clauses), Nonograms (40 hand-drawn plates, deduction only).
